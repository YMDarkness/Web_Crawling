...
def market_xgboost(df_market):
    ...
    USD_X_train, USD_X_test, USD_y_train, USD_y_test = train_test_split(USD_X, USD_y, test_size=0.2, random_state=0.2)
    ...
    print(f"[XGBoost 결과] RMSE : {USD_rmse:.4f}\n")
    ...
    plt.title('XGBoost 모델을 이용한 환율 변화 예측')
    plt.legend()
    plt.show()
    ...
    JPY_X_train, JPY_X_test, JPY_y_train, JPY_y_test = train_test_split(JPY_X, JPY_y, test_size=0.2, random_state=0.2)
    ...
    print(f"[XGBoost 결과] RMSE : {JPY_rmse:.4f}")
